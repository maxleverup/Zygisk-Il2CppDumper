...
// Global variable to pass target_library into JNI_OnLoad via ARM native bridge
static std::string g_target_library = "libil2cpp.so";

void SetTargetLibrary(const char *lib) {
    g_target_library = lib;
    LOGI("SetTargetLibrary: %s", g_target_library.c_str());
}
...
bool NativeBridgeLoad(const char *game_data_dir, int api_level, void *data, size_t length, const char *target_library) {
    //TODO wait for houdini initialization
    sleep(5);
    ...
    if (nb) {
        ...
        if (callbacks) {
            ...
            if (arm_handle) {
                ...
                // Set global BEFORE calling JNI_OnLoad so ARM side can read it
                SetTargetLibrary(target_library);
                init(vms, (void *) game_data_dir, nullptr);
                return true;
            }
            close(fd);
        }
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    // Read from global variable set by NativeBridgeLoad on x86 side
    // On real ARM devices, fall back to reading config file
    const char *target_lib = g_target_library.c_str();
    if (g_target_library == "libil2cpp.so") {
        // Possibly running on real ARM, read config
        Config config = ReadConfig();
        target_lib = config.targetLibrary.c_str();
        LOGI("JNI_OnLoad (ARM): using config library: %s", target_lib);
        std::thread hack_thread(hack_start, game_data_dir, std::string(target_lib));
        hack_thread.detach();
    } else {
        LOGI("JNI_OnLoad (NativeBridge): using global library: %s", target_lib);
        std::thread hack_thread(hack_start, game_data_dir, std::string(target_lib));
        hack_thread.detach();
    }
